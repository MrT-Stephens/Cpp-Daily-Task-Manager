#include "../lib/easy-encryption/Base64.h"
#include "../lib/easy-encryption/vigenere.h"

inline std::string encrypt(std::string& msg, std::string& key)
{
    Base64 base_64;
    std::vector<char> msg2(msg.begin(), msg.end());
    std::string b64_str = base_64.encode(msg2);
...
inline std::string sanitize_utf8(std::string& str)
{
    int i, f_size = str.size();
    unsigned char c = NULL, c2 = NULL, c3 = NULL, c4 = NULL;
    string to;
    to.reserve(f_size);
...
inline std::string decrypt(std::string& encrypted_msg, std::string& key)
{
    ...
    std::vector<char> b64_decode_vec = base_64.decode(b64_encoded_str);
